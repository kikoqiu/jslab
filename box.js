...
box.loadNDArray=async function(){
  if(!globalThis.ndarray){
    ...
    ndarray.NDArray.prototype.operatorMod=ndarray.NDArray.prototype.mod;
    ...
    ndarray.NDArray.prototype.operatorNeg=ndarray.NDArray.prototype.neg;
    ndarray.NDArray.prototype.operatorBitwiseAnd=ndarray.NDArray.prototype.bitwise_and;
    ndarray.NDArray.prototype.operatorBitwiseOr=ndarray.NDArray.prototype.bitwise_or;
    ndarray.NDArray.prototype.operatorBitwiseXor=ndarray.NDArray.prototype.bitwise_xor;
    ndarray.NDArray.prototype.operatorBitwiseLShift=ndarray.NDArray.prototype.bitwise_lshift;
    ndarray.NDArray.prototype.operatorBitwiseRShift=ndarray.NDArray.prototype.bitwise_rshift;
    ndarray.NDArray.prototype.operatorBitwiseURShift=ndarray.NDArray.prototype.bitwise_rshift;
    ndarray.NDArray.prototype.operatorBitwiseNot=ndarray.NDArray.prototype.bitwise_not;
...
/**
 * Render markdown content to HTML and display it.
 * @param {string} md Markdown content
 * @param {boolean} prevLinks Previous links dictionary for reference-style links
 */
box.markdown = function(md, prevLinks) {
  const html = box.markdown2HTML(md, prevLinks);
  box.echoHTML(`<div class="md">${html}</div>`);
};
 ...
 */
box.markdown2HTML = function (md, prevLinks = {}) {
  const TAGS = {
    '': ['<em>','</em>'],
    '_': ['<strong>','</strong>'],
    '*': ['<strong>','</strong>'],
  ...
  function encodeAttr(str) {
	...
	while ( (token=tokenizer.exec(md)) ) {
		...
		// > Quotes, -* lists:
			...
			inner = box.markdown2HTML(outdent(token[5].replace(/^\s*[>*+.-]/gm, '')));
			if (t=='>') t = 'blockquote';
			else {
				t = t.match(/\./) ? 'ol' : 'ul';
				inner = inner.replace(/^(.*)(\n|$)/gm, '<li>$1</li>');
		...
		// Headings:
			...
			chunk = '<'+t+'>' + box.markdown2HTML(token[12] || token[15], links) + '</'+t+'>';
		}
		// `code`:
		else if (token[16]) {
